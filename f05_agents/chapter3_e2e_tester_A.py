# f05_agents/chapter3_e2e_tester_A.py (12)
#
# Created:
#     1405/06/19
#
# Run: pytest -v -s f05_agents/chapter3_e2e_tester_A.py
#
...
class FixedSymbolPolicy:
    ...
    این Policy فقط برای validation استفاده می‌شود
    و نماینده مدل RL نهایی نیست.
    """
...
class FixedMetaPolicy:
    ...
    Meta-Agent باید خروجی Symbol-Agent ها را
    در قالب PortfolioDecision مدیریت کند.
    """
...
def test_decision_to_portfolio_action():
    ...
    assert by_symbol["XAUUSD"].target_lots == pytest.approx(
        1.50
    )

    assert by_symbol["EURUSD"].target_side == -1
    assert by_symbol["EURUSD"].target_lots == pytest.approx(
        0.80
    )

    assert by_symbol["GBPUSD"].target_side == 1
    assert by_symbol["GBPUSD"].target_lots == pytest.approx(
        0.60
    )
...
def test_portfolio_state_adapter_feeds_meta_agent():
    ...
    assert context.equity == pytest.approx(
        10_000.0
    )

    assert context.balance == pytest.approx(
        10_000.0
    )

    assert context.exposure["XAUUSD"] == pytest.approx(
        0.10
    )

    assert context.exposure["EURUSD"] == pytest.approx(
        -0.05
    )
...
def test_risk_block_propagates_to_final_decision():
    ...
    assert "portfolio_risk_blocked" in (
