# f05_agents/decision_engine_tester_A.py (7)
#
# Run:
#     pytest -v -s f05_agents/decision_engine_tester_A.py
...
def test_symbol_isolation():
    ...
    xau_agent = engine.get_symbol_agent(
        "XAUUSD"
    )

    eur_agent = engine.get_symbol_agent(
        "EURUSD"
    )

    assert (
        xau_agent.state.last_signal == 1
    )

    assert (
        eur_agent.state.last_signal == -1
    )
...
def test_missing_symbol_is_rejected():
    ...
    try:
        ...
        raise AssertionError(
            "Missing symbol was not rejected"
        )
...
def test_decision_cycle_requires_shared_timestamp():
    ...
    try:
        ...
        raise AssertionError(
            "Mismatched timestamps were not rejected"
        )
...
def test_decision_cycle_requires_portfolio_timestamp_match():
    ...
    try:
        ...
        raise AssertionError(
            "Portfolio timestamp mismatch was not rejected"
        )
