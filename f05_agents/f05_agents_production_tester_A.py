# f05_agents/f05_agents_production_tester_A.py (13)
#
# Created:
#     1405/06/19
#
# Run: pytest -v -s f05_agents/f05_agents_production_tester_A.py
#
...
def test_model_lineage_is_preserved():
    ...
    assert decision.model.model_name == (
        "meta-policy"
    )

    assert decision.model.model_version == (
        "1.0.0"
    )

    assert decision.model.policy_version == (
        "meta-policy-1"
    )

    assert decision.model.config_version == (
        "cfg-1"
    )

    assert decision.model.experiment_id == (
        "prod-validation"
    )
...
def test_final_decision_to_environment_action():
    ...
    assert xau.target_lots == pytest.approx(
        1.50
    )

    assert eur.target_side == -1
    assert eur.target_lots == pytest.approx(
        0.80
    )
...
def test_identical_decision_cycles_are_reproducible():
    ...
    decision_a = engine_a.decide(
        **kwargs
    )

    decision_b = engine_b.decide(
        **kwargs
    )
