# f05_agents/portfolio_context_adapter_tester_A.py (11)
#
# Run:
#     pytest -v -s f05_agents/portfolio_context_adapter_tester_A.py
...
def test_portfolio_state_to_context():
    ...
    assert context.used_margin == pytest.approx(
        1_000.0
    )

    assert context.free_margin == pytest.approx(
        9_150.0
    )

    assert context.margin_level == pytest.approx(
        10.15
    )
...
def test_correlation_and_concentration_are_preserved():
    ...
    assert context.concentration[
        "XAUUSD"
    ] == pytest.approx(0.20)

    assert context.correlation[
        "XAUUSD"
    ]["EURUSD"] == pytest.approx(0.92)
