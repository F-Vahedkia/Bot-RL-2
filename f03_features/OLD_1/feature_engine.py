...
def _compute_feature_df_v2(
    ...
    if fn is not None and hasattr(ps, 'mode') and ps.mode and not fn.supports(ps.mode):
        logger.warning("Feature '%s' does not support mode '%s'. Skipping.", ps.name, ps.mode)
        return pd.DataFrame(index=df_in.index)
    # --- END PATCH ---
    # --- PATCH: required columns validation ---
    if fn is not None and hasattr(fn, 'required_cols') and fn.required_cols:
        missing = [c for c in fn.required_cols if c not in df_in.columns]
        if missing:
            raise KeyError(
                f"Feature '{ps.name}' requires columns {missing} missing from DataFrame."
            )
    # --- END PATCH ---
    ...
    if ps.name in {"fibo_features_full", "fibo_run"}:
        ...
        # --- PATCH: mode validation (MTF path) ---
        _spec = get_indicator(ps.name)
        if _spec is not None and hasattr(ps, 'mode') and ps.mode and not _spec.supports(ps.mode):
            logger.warning("Feature '%s' does not support mode '%s'. Skipping.", ps.name, ps.mode)
            return pd.DataFrame(index=df_in.index)
        # --- END PATCH ---
...
def apply_representation(feature_df: pd.DataFrame, representation_config: dict) -> pd.DataFrame:
    ...
    Suppose:
            feature_df = engine collects raw features
...
1  _available_timeframes             --  --  --  --  --  --  ok  ok  --  --
2  _ensure_utc_index                 --  --  --  --  ok  ok  ok  ok  --  --
3  _extract_base_ohlc_from_plain_df  --  --  --  --  --  --  ok  --  --  --
4  _tf_to_rule                       --  --  --  --  ok  --  --  --  --  --
5  _resample_ohlc_from_base          --  --  --  --  --  --  ok  --  --  --
6  _merge_on_base                    --  --  --  --  --  --  --  ok  --  --
7  _compute_feature_df_v2            --  --  --  --  --  --  --  ok  --  --
8  run_specs_v2                      --  --  --  --  --  --  --  --  --  ok
9  apply_representation              --  --  --  --  --  --  --  --  --  -- NOT USED
10 _entry                            --  --  --  --  --  --  --  --  --  -- MIAN
"""