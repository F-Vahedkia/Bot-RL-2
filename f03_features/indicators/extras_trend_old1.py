# f03_features/indicators/extras_trend_old1.py
...
#------------------------------------------------------------------------------
""" این تابع محاسبه‌کنندهٔ هم‌زمان موارد زیر است
+DI (Positive Directional Indicator)
-DI (Negative Directional Indicator)
ADX (Average Directional Index)
ADXR (Average Directional Index Rating)
یعنی کل خانوادهٔ Directional Movement System (Welles Wilder) را یک‌جا می‌سازد.
"""
...
def kama(s: pd.Series, n: int = 10, fast: int = 2, slow: int = 30) -> pd.Series:
    s = s.astype("float32")  # جلوگیری از downcast بی‌سروصدا
    ef = (s.diff(n).abs()) / (s.diff().abs().rolling(n, min_periods=n).sum().replace(0, np.nan))
    sc = ((ef * (2/(fast+1) - 2/(slow+1)) + 2/(slow+1))**2).astype("float32")
    out = pd.Series(index=s.index, dtype="float32")
    out.iloc[:n] = s.iloc[:n].astype("float32")
...
def rsi_zone(
    ...
    rsi = rsi(close, length=period).astype("float32")
