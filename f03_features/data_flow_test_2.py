...
def _assert_indicator_observation_complete(
    ...
    *,
    ...
    assert not missing, (
        f"{mode} Observation is missing Indicator columns.\n"
        f"Missing : {missing}\n"
        f"Actual  : {actual_columns}"
    )

    # Indicator columns must remain the first block of Observation.
    # Candle columns, when enabled, are appended after them.
    indicator_count = len(expected_columns)

    assert actual_columns[:indicator_count] == expected_columns, (
        f"{mode} Indicator column order is not correct.\n"
        f"Expected Indicator columns : {expected_columns}\n"
        f"Actual leading columns     : "
        f"{actual_columns[:indicator_count]}\n"
        f"Actual full columns        : {actual_columns}"
    )

    # Explicit freeze-contract check for the current
    # 23-indicator / M1-M2 configuration.
    assert len(specs) == 46, (
    ...
    # Observation may now contain additional Candle-window columns.
    assert len(actual_columns) >= 80, (
        f"{mode} Observation must contain at least 80 Indicator columns, "
