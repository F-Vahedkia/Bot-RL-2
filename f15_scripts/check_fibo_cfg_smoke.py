...
def _deep_get(d: Dict[str, Any], path: str, default: Any = None) -> Any:
    """برداشت امن مقدار از دیکشنری تودرتو با dot-notation"""
    cur: Any = d
    for p in path.split("."):
...
def resolve_fibo_params_from_config() -> FiboResolvedParams:
    ...
    fibo = _deep_get(cfg_all, "features.fibonacci", {}) or {}

    # نسبت‌های رتریسمنت
    retr = _deep_get(fibo, "retracement_ratios", None)
    ...
    # نسبت‌های اکستنشن
    exts = _deep_get(fibo, "extension_ratios", None)
    ...
    # نسبت‌های زون طلایی
    gz = _deep_get(fibo, "golden_zone.ratios", None)
    ...
    cluster_tol_pct = float(_deep_get(fibo, "cluster.tol_pct", 0.08))
    cluster_prefer_ratio = float(_deep_get(fibo, "cluster.prefer_ratio", 0.618))
    tfw = _deep_get(fibo, "cluster.tf_weights", None)
    ...
    cluster_w_trend = float(_deep_get(fibo, "cluster.w_trend", 10.0))
    cluster_w_rsi   = float(_deep_get(fibo, "cluster.w_rsi", 10.0))
    cluster_w_sr    = float(_deep_get(fibo, "cluster.w_sr", 10.0))
    cluster_sr_tol_pct = float(_deep_get(fibo, "cluster.sr_tol_pct", 0.05))

    # SL ATR Mult (برای fib_ext_targets_cfg)
    sl_atr_mult = float(_deep_get(fibo, "sl_atr_mult", 1.5))
...
def demo_fib_ext_targets_cfg(p: FiboResolvedParams, entry: float, leg_low: float, leg_high: float, side: str) -> Optional[pd.DataFrame]:
    """
