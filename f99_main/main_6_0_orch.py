...
class BotOrchestrator:
    ...
    def _on_data_updated(self, df: pd.DataFrame) -> None:
        """این متد توسط DataHandler از طریق set_data_callback صدا زده می‌شود"""
        if self._running:
    ...
    def run_batch_loop(self):
        """حلقه برای حالت‌های بچ (پیمایش رکوردها) - این حلقه polling نیست، پیمایش تاریخی است."""
        while self._running and self.data_source.has_next():
            ...
            if candle:
                ...
                # در اینجا می‌توان features را محاسبه کرد
            else:
                break
        self._running = False
