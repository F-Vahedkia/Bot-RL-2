# f02_data/data_handler_F_3.py (3)
#
...
class BuildParams:
    ...
    def __init__(self,
        ...
        self.timeframes = _other_tfs # <=== توجه شود: در خروجی این کلاس- برخلاف ورودی آن- این پارامتر شامل تایمفریمهای غیر پایه است.
...
class DataHandler:
    ...
    def __init__(
        ...
        # }
        #
        # If required_bars is not supplied, only __warmups_dicts is
        # used as a backward-compatible fallback.
        if required_bars is None:
            try:
                source_required_bars = self.cfg["__warmups_dicts"][symbol]
            except KeyError as ex:
                raise ValueError(
                    f"Required bars for symbol '{symbol}' were not provided "
                    f"and '{symbol}' was not found in cfg['__warmups_dicts']."
                ) from ex
        else:
        ...
        # -----------------------------------------------------------
        # Normalize timeframe names and validate bar counts.
        # -----------------------------------------------------------
        normalized_required_bars: Dict[str, int] = {}
        ...
        for tf, bars in source_required_bars.items():
            ...
            tf_key = tf.upper().replace(" ", "")

            # Convert aliases to the project's canonical timeframe name.
            canonical_tf = _TF_MAP.get(tf_key)

            # Also allow a canonical TF directly if it is already present
            # in _TF_MINUTES.
            if canonical_tf is None and tf_key in _TF_MINUTES:
                canonical_tf = tf_key

            if canonical_tf is None:
                raise ValueError(
                    f"Invalid timeframe '{tf}' in required_bars."
                )

            # Prevent silent overwriting when two aliases refer to the
            # same canonical timeframe with different bar requirements.
            if canonical_tf in normalized_required_bars:
                previous_bars = normalized_required_bars[canonical_tf]

                if previous_bars != bars:
                    raise ValueError(
                        f"Conflicting required bar counts for timeframe "
                        f"'{canonical_tf}': {previous_bars} and {bars}."
                    )

                continue

            normalized_required_bars[canonical_tf] = bars
        ...
        # the base timeframe.
        # -----------------------------------------------------------
        first_tf = next(iter(normalized_required_bars))
        remaining_tfs = [
            tf for tf in normalized_required_bars
            if tf != first_tf
        ]
        ...
        self._warmup_dict: Dict[str, int] = {
            tf: normalized_required_bars[tf]
    ...
    def _inspect_raw_metadata(
        ...
        for tf in timeframes:
            tf = tf.upper()
            meta_path = self.raw_dir / symbol / f"{tf}.meta.json"
            info = {
                "status": "missing",
                "rows": 0,
                "first_index": None,
            ...
            try:
                ...
                logger.warning("Invalid metadata for %s/%s: %s", symbol, tf, exc)
                info["status"] = "invalid"
            result[tf] = info
    ...
    def download_required_data(
        ...
        # ---------------------------------------------------------
        if params.mode == "number":
            if params.start_lastrows is None:
                # هیچ پنجره عددی مشخصی در BuildParams تعیین نشده است.
                # در این حالت نباید lookback_bars=0 ارسال شود.
                # اجازه می‌دهیم MT5DataLoader_batch از تنظیمات پیش‌فرض
                # خودش برای محدوده دانلود استفاده کند.
    ...
    def build_old(self, params: BuildParams) -> MTFDataset:
    ...
    # --- نسخه جدید که از دو تابع Batch-3, Batch-4 بالا استفاده میکند ---
    def build(self, params: BuildParams) -> MTFDataset:
        ...
        # -------------------------------------------------
        metadata_info = self._inspect_raw_metadata(
            symbol=symbol,
            timeframes=all_tfs,
            params=params,
        )
