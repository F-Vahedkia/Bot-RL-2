...
# روش اجری برنامه:
# pytest f16_tests/unit/test_adv_indicators.py -q -rA --disable-warnings --durations=20
...
# -- مسیر پروژه (درج ریشه) ----------------------------------------------------
_PR = Path(__file__).resolve().parents[2]  # ریشه‌ی مخزن (شامل f04_features/)
if str(_PR) not in sys.path:
    sys.path.insert(0, str(_PR))

# -- وارد کردن ماژول‌ها؛ در صورت خطا، کل ماژول را Skip کن ----------------------
try:
    from f04_features.indicators import engine as _engine
    from f04_features.indicators.registry import (
        get_indicator_v2,
...
def test_registry_reports_advanced_tags():
    ...
    """
    tags = list_all_indicators_v2(include_legacy=False)
    for key in ("adr", "adr_distance_to_open", "sr_overlap_score", "round_levels"):
        assert key in tags and tags[key] == "advanced", f"missing or not advanced: {key}"


def test_get_indicator_v2_returns_callables():
    """
    get_indicator_v2 باید callable بدهد (آداپترها ثبت شده‌اند).
    """
    for key in ("adr", "adr_distance_to_open", "sr_overlap_score", "round_levels"):
        fn = get_indicator_v2(key)
