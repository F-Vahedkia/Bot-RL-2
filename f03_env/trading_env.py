...
# [LOGLEVEL:ENV] — اگر F03_ENV_DEBUG=1 باشد، لاگ‌های DEBUG این ماژول فعال می‌شوند
if str(os.getenv("F03_ENV_SILENT", "0")) == "1":
    # Persian: حالت سایلنت — فقط WARNING و بالاتر
    logger.setLevel(logging.WARNING)
elif str(os.getenv("F03_ENV_DEBUG", "0")) == "1":
...
class TradingEnv(BaseTradingEnv):
    ...
    def _log_trade_csv(self, reason:str, side:str, entry:float, exit_:float, lots:float,
        ...
        """Persian: خروجی یک ترید بسته‌شده در CSV برای دیباگ. مسیر پیش‌فرض را با ENV: F03_TRADE_CSV می‌توان تغییر داد."""
        try:
            default_path = os.path.join(str(_project_root()), "f16_tests", "trades_debug.csv")
    ...
    def _make_obs(self, t: int) -> np.ndarray:
        ...
        if self.env_cfg.normalize and self.scaler is not None and len(frame) > 0:
            ...
            frame[self.scale_cols] = self.scaler.transform(frame[self.scale_cols])
    ...
    def step(self, action: int) -> StepResult:
        ...
        if 0 <= raw_action <= 4:  # حالت 5 مقداری: 0..4 ↦ -2..+2
            _map = (-2, -1, 0, +1, +2)
            a5 = _map[raw_action]
            # فقط اکسستریم‌ها (±2) اجازه‌ی ورود از حالت فلت را دارند
            new_pos = 0 if (self._pos == 0 and abs(a5) < 2) else (1 if a5 > 0 else (-1 if a5 < 0 else 0))
        else:
        ...
        if self._pos!=0 and self._entry_price is not None and t_global+1<=len(self.df_full)-1 and getattr(self,"_pip_size",0.0)>0.0:
