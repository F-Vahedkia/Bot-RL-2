# f15_testcheck/unit/ts03_idx_zzg__1zigzag_new_1D.py
# Run: python -m f15_testcheck.unit.ts03_idx_zzg__1zigzag_new_1D

import numpy as np
import pandas as pd
from datetime import datetime
from f03_features.indicators.zigzag import (
    _zigzag_mql_numpy_complete          as zig_core_1,
    _zigzag_mql_njit_loopwise_complete  as zig_core_2,
    _zigzag_numpy                       as zig_core_3,
    _zigzag_njit                        as zig_core_4,
)
_PATH = "f16_test_results/"
_PATH = ""

depths = np.arange(8,35)
deviation = 0.05
# backstep = 10
...
# ============================================================
zigzag_funcs = {
                # "zig_core_1": zig_core_1,
                # "zig_core_2": zig_core_2,
                "zig_core_3": zig_core_3,
                # "zig_core_4": zig_core_4,
                }
    ...
    for key in zigzag_funcs.keys():
        ...
        state, high_actual, low_actual, confirmed_at, developing_leg = \
            func(
                df["high"].values,
                df["low"].values,
                depth=depth, deviation=deviation, backstep=depth,
            )
        t2 = datetime.now()
        ...
        #     }
        # )
        # df_new.reset_index().to_csv(f"{_PATH}ts03_idx_zzg__1zigzag_new_1D_{key}.csv", index_label="no.")

        # --- Check functions ---
        violations, fault_poss = check_zigzag_extrema_consistency(df["high"].values, df["low"].values, state)
        ...
        violations, fault_poss = check_zigzag_triplet_order(df["high"].values, df["low"].values, state)
        check[key + "_trip"] = violations
        # trip_fault_poss[key] = fault_poss


    fault_dict[f"{depth}"] = check.sum(axis=0)

print(pd.DataFrame(fault_dict))
print("\n")
print(pd.DataFrame(fault_dict).sum(axis=0))
# -----------------------------------------------
# saving chech results to .csv
# -----------------------------------------------
# check.to_csv(f"{_PATH}ts03_idx_zzg__1zigzag_new_1D_check.csv", index_label="no.")
